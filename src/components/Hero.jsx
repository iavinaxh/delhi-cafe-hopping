  ...
  return (
    <div className="relative overflow-hidden bg-[#FFF4E6] text-[#3A1F14] py-14 sm:py-24 border-b border-[#E7B894]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="hero-support-row mt-10 max-w-3xl"><div className="hero-support-search"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#CC3A63] pointer-events-none"/><input type="text" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Browse by cafe, restaurant, area or vibe..." className="w-full pl-12 pr-10 py-4 rounded-2xl bg-white/80 border border-[#E7B894] text-[#3A1F14] placeholder-[#A4816E] focus:outline-none focus:ring-2 focus:ring-[#CC3A63]/30 focus:border-[#CC3A63] text-sm sm:text-base shadow-[0_8px_24px_rgba(78,43,25,0.08)]"/>{searchQuery&&<button onClick={()=>setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-[#7A2E12] bg-[#FCE6D0] px-2 py-1 rounded-lg">Clear</button>}</div><FoodDeliverySticker /></div>
      </div>
    </div>
  );
}
