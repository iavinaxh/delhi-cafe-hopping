...
import { Search, Sparkles, MapPin, Compass, ShieldCheck } from 'lucide-react';
    ...
    <div className="relative overflow-hidden bg-[#FFF7EB] text-[#332B23] py-12 sm:py-20 border-b border-[#A2AB73]/35">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#F9F0E0] border border-[#A2AB73]/45 text-[#56602E] text-xs sm:text-sm font-medium mb-6 shadow-sm">
          <Sparkles className="w-4 h-4 text-[#CC3A63]" /><span>Delhi NCR Date & Cafe Guide</span><span className="w-1.5 h-1.5 rounded-full bg-[#A2AB73]"></span>
        ...
        <p className="mt-4 sm:mt-6 text-base sm:text-lg text-[#685D50] max-w-3xl mx-auto font-sans leading-relaxed">A curated guide to Delhi NCR's best cafe dates, from cozy coffee corners to lively dinner spots, with honest budgets, easy routes, and ideas for what to do after you eat.</p>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-xs text-[#685D50]">
          <span className="px-3 py-1 rounded-full bg-[#F9F0E0] border border-[#A2AB73]/35 flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-[#CC3A63]" /> Honest budgets & food picks</span>
          <span className="px-3 py-1 rounded-full bg-[#F9F0E0] border border-[#A2AB73]/35 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-[#56602E]" /> Direct map & route links</span>
          <span className="px-3 py-1 rounded-full bg-[#F9F0E0] border border-[#A2AB73]/35 flex items-center gap-1.5"><Compass className="w-3.5 h-3.5 text-[#CC3A63]" /> Zomato & Dineout links</span>
        </div>
        <div className="mt-8 max-w-2xl mx-auto"><div className="relative flex items-center"><Search className="absolute left-4 w-5 h-5 text-[#CC3A63] pointer-events-none" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search cafes, zones, vibes, or dish recommendations..." className="w-full pl-12 pr-10 py-4 rounded-2xl bg-white/80 border border-[#A2AB73]/50 text-[#332B23] placeholder-[#958876] focus:outline-none focus:ring-2 focus:ring-[#CC3A63]/35 focus:border-[#CC3A63] text-sm sm:text-base shadow-[0_8px_24px_rgba(78,62,38,0.08)] transition-all" />{searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-4 text-xs font-semibold text-[#56602E] hover:text-[#CC3A63] bg-[#F9F0E0] px-2 py-1 rounded-lg">Clear</button>}</div></div>
