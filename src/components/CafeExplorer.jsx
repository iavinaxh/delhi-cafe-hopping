...
export default function CafeExplorer({ cafes, searchQuery, wishlist, onToggleWishlist }) {
  ...
  return <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-[#FFF4E6]">
    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-7">
      <div><p className="text-xs uppercase tracking-[0.18em] text-[#CC3A63] font-bold">Explore the catalogue</p><h2 className="font-serif text-3xl sm:text-4xl font-bold text-[#3A1F14]">Cafes & restaurants worth knowing</h2><p className="text-sm text-[#6F5041] mt-2 max-w-2xl">Browse compact cards. Click any cafe or restaurant to reveal the full details, routes and direct links.</p></div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 items-start">{filtered.map(c=><CafeCard key={c.id} cafe={c} isWishlisted={wishlist.includes(c.id)} onToggleWishlist={onToggleWishlist}/>)}</div>
  </section>;
}
