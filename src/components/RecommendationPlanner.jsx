...
const LOCATION_ALIASES={
  ...
  'Khan Market':['khan market'],'Saket':['saket'],'Hauz Khas':['hauz khas','hkv'],
  'Greater Kailash':['gk1','gk2','gk3','greater kailash'],'Defence Colony':['defence colony'],
  'Malviya Nagar':['malviya nagar'],'Lajpat Nagar':['lajpat nagar'],'Nehru Place':['nehru place'],
  'Chanakyapuri':['chanakyapuri'],'Chhatarpur':['chhatarpur'],'Rajouri Garden':['rajouri garden'],
  'Punjabi Bagh':['punjabi bagh'],'Janakpuri':['janakpuri'],'Dwarka':['dwarka'],
  'Noida':['noida'],'Gurgaon':['gurgaon','gurugram','cyber city','golf course road']
};
...
function Detail({label,children,icon}){if(!children)return null;return <div className="rounded-2xl bg-[#FFF7EB] border border-[#A2AB73]/25 p-3.5"><span className="text-[#CC3A63] font-bold flex items-center gap-1 mb-1 text-[10px] uppercase tracking-wider">{icon}{label}</span><p className="text-xs sm:text-sm text-[#4E453B] leading-relaxed">{children}</p></div>;}

function RecommendationCard({p,people,index}){
 const [open,setOpen]=useState(false);const [hovered,setHovered]=useState(false);const expanded=open||hovered;const hasDineout=Boolean(p.dineoutUrl);const id=p.id||`${p.name}-${index}`;
 const stop=e=>e.stopPropagation();
 return <article onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onClick={()=>setOpen(v=>!v)} className={`group rounded-3xl bg-[#F9F0E0] border overflow-hidden shadow-lg flex flex-col cursor-pointer transition-all duration-300 ${expanded?'border-[#CC3A63]/60 shadow-[0_18px_45px_rgba(78,62,38,0.18)]':'border-[#A2AB73]/50 hover:-translate-y-1'}`} aria-expanded={expanded}>
   <div className="p-5">
     <div className="flex items-start justify-between gap-3"><div><span className="text-[10px] uppercase tracking-wider font-bold text-[#CC3A63]">{p.matchLabel}</span><h4 className="font-serif text-xl font-bold text-[#332B23] mt-1">{p.name}</h4><p className="text-xs text-[#786C5C] mt-1 flex items-center gap-1"><MapPin className="w-3 h-3"/>{p.zone}</p></div><span className="shrink-0 text-[10px] font-bold px-2 py-1 rounded-full bg-[#A2AB73]/20 border border-[#A2AB73]/50 text-[#56602E]">2 people</span></div>
     <div className="mt-4 flex items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-[#786C5C]">Listed budget</p><p className="text-sm font-bold text-[#56602E]">{p.budget} <span className="font-normal text-[#786C5C]">/ 2 people</span></p></div><span className="text-[10px] text-[#786C5C]">Tap / hover for details</span></div>
   </div>
   <div className={`grid transition-[grid-template-rows] duration-300 ${expanded?'grid-rows-[1fr]':'grid-rows-[0fr]'}`}><div className="overflow-hidden min-h-0"><div className="px-5 pb-5 space-y-3 border-t border-[#A2AB73]/25 pt-4">
      <div className="rounded-2xl bg-[#A2AB73]/15 border border-[#A2AB73]/35 p-3.5"><p className="text-[10px] uppercase tracking-wider font-bold text-[#56602E]">Budget for {people} {people===1?'person':'people'}</p><p className="text-xs sm:text-sm text-[#4E453B] mt-1 leading-relaxed">{p.budgetFit}</p><p className="text-[10px] text-[#786C5C] mt-2">The catalogue price above is the original 2-person estimate. The estimate here is scaled to your selected group size.</p></div>
   ...
   <div className="px-5 pb-4"><div className="flex items-center justify-center gap-1 text-[10px] font-bold text-[#CC3A63]">{expanded?'Hide details':'View cafe details'} <ChevronDown className={`w-3.5 h-3.5 transition-transform ${expanded?'rotate-180':''}`}/></div></div>
   <div onClick={stop} className={`grid ${hasDineout?'grid-cols-4':'grid-cols-3'} gap-2 p-4 pt-0 border-t border-[#A2AB73]/30 bg-[#F9F0E0]`}>
      <a href={p.mapsUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-xl bg-[#A2AB73] hover:bg-[#8F995F] text-white text-[10px] font-bold text-center transition-colors"><MapPin className="w-3 h-3 mx-auto mb-1"/>MAPS</a>
      <a href={p.zomatoUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-xl bg-[#CC3A63] hover:bg-[#B52F55] text-white text-[10px] font-bold text-center transition-colors">ZOMATO</a>
      {p.websiteUrl&&<a href={p.websiteUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-xl bg-[#FFF7EB] text-[#56602E] border border-[#A2AB73]/40 text-[10px] font-bold text-center"><Globe className="w-3 h-3 mx-auto mb-1"/>WEB</a>}
      {hasDineout&&<a href={p.dineoutUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-xl bg-[#56602E] hover:bg-[#414923] text-white text-[10px] font-bold text-center"><ShoppingBag className="w-3 h-3 mx-auto mb-1"/>DINEOUT</a>}
   </div>
 </article>;
}
...
export default function RecommendationPlanner({cafes=[]}){
 ...
 <div className="md:col-span-2 lg:col-span-3 rounded-2xl bg-[#FFF7EB] border border-[#A2AB73]/50 p-4 flex items-center gap-4"><div className="shrink-0 w-10 h-10 rounded-xl bg-[#A2AB73]/20 flex items-center justify-center"><Calculator className="w-5 h-5 text-[#56602E]"/></div><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#56602E]">Your budget plan</p><p className="text-sm sm:text-base font-bold text-[#332B23]">{formatMoney(selectedBudget)} total for {people} {people===1?'person':'people'} <span className="font-normal text-[#786C5C]">• {formatMoney(budgetPerPerson)} per person</span></p><p className="text-xs text-[#786C5C] mt-1">Venue prices in this guide are listed for 2 people. We scale the estimate automatically when your group size changes.</p></div></div>
 ...
 {searched&&<div className="mt-9"><div className="flex flex-wrap items-end justify-between gap-3 mb-5"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#FFF7EB]/80">CURATED DELHI NCR GUIDE</p><h3 className="font-serif text-2xl sm:text-3xl font-bold text-[#241F1A]">Your best matches</h3><p className="text-xs text-[#FFF7EB]/80 mt-1">Each card starts compact. Hover on desktop or tap a card on mobile to see the full venue details.</p></div><span className="text-xs bg-[#F9F0E0] text-[#56602E] px-3 py-1.5 rounded-full border border-[#A2AB73]/50">{results.length} suggestions</span></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">{results.map((p,i)=><RecommendationCard key={p.id||`${p.name}-${i}`} p={p} people={people} index={i}/>)}</div></div>}
 </div></section>;
}
