...
export default function CafeCard({ cafe, isWishlisted, onToggleWishlist, onSelectCafe }) {
  ...
  return (
    <article
      className={`group w-full rounded-[28px] bg-[#FFF4E6] border overflow-hidden transition-all duration-300 self-start cursor-pointer ${expanded ? 'border-[#CC3A63] shadow-[0_18px_42px_rgba(89,37,15,0.16)]' : 'border-[#E7B894] shadow-[0_8px_24px_rgba(89,37,15,0.08)] hover:-translate-y-1'}`}
    ...
    >
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={(event) => { stop(event); onToggleWishlist(cafe.id); }} aria-label={`Save ${cafe.name}`} className="shrink-0 p-2.5 rounded-full bg-[#FCE6D0] border border-[#E7B894] hover:border-[#CC3A63]/60 transition-colors"><Heart className={`w-4 h-4 ${isWishlisted ? 'fill-[#CC3A63] text-[#CC3A63]' : 'text-[#7A2E12]'}`} /></button>
        </div>
      ...
      <div className={`md:hidden grid transition-[grid-template-rows] duration-300 ${expanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
        <div className="overflow-hidden min-h-0">
          <div className="px-5 pb-5 pt-4 border-t border-[#E7B894] bg-[#FCE6D0]/45 space-y-3">
            <div className="p-3.5 rounded-2xl bg-[#FFF4E6] border border-[#E7B894]"><span className="text-[#CC3A63] font-bold flex items-center gap-1 mb-1 text-[10px] uppercase tracking-wider"><Utensils className="w-3.5 h-3.5" /> WHAT TO ORDER</span><p className="text-xs text-[#4E382E] leading-relaxed">{cafe.whatToOrder}</p></div>
            <div className="p-3.5 rounded-2xl bg-[#FFF4E6] border border-[#E7B894]"><span className="text-[#7A2E12] font-bold flex items-center gap-1 mb-1 text-[10px] uppercase tracking-wider"><Route className="w-3.5 h-3.5" /> GETTING THERE</span><p className="text-xs text-[#4E382E] leading-relaxed">{cafe.metroRoute}</p></div>
            <div className="p-3.5 rounded-2xl bg-[#FCE6D0] border border-[#E7B894]"><span className="text-[#7A2E12] font-bold block mb-1 text-[10px] uppercase tracking-wider">BEST KNOWN FOR</span><p className="text-xs text-[#4E382E] leading-relaxed">{cafe.bestFor}</p></div>
            {cafe.curatorTake && <div className="p-3.5 rounded-2xl bg-[#7A2E12]/5 border border-[#E7B894]"><span className="text-[#CC3A63] font-bold block mb-1 text-[10px] uppercase tracking-wider">REAL TALK</span><p className="text-xs text-[#4E382E] leading-relaxed">{cafe.curatorTake}</p></div>}
          </div>
        </div>
      </div>
      ...
      {expanded && <div className={`md:hidden grid ${hasDineout ? 'grid-cols-4' : 'grid-cols-3'} gap-2 p-4 border-t border-[#E7B894] bg-[#FCE6D0]`} onClick={stop}>
        ...
        {hasDineout && <a href={cafe.dineoutUrl} target="_blank" rel="noopener noreferrer" className="min-h-[44px] rounded-xl bg-[#56602E] text-white text-[10px] font-bold text-center flex flex-col items-center justify-center gap-0.5"><ShoppingBag className="w-3.5 h-3.5" />DINEOUT</a>}
      </div>}
    </article>
  );
}
